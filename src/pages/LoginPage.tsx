...
export function LoginPage() {
  ...
  const onSubmit = async (data: Form) => {
    try {
      ...
      nav(params.get('from') ?? '/', { replace: true })
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-[420px] xl:w-[500px] bg-[#FFFBEB] border-r border-border flex-col justify-between p-12 shrink-0">
        ...
        <div>
          ...
          <p className="text-sm text-fg/40">— Neil Gaiman</p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-bg">
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-3xl font-serif font-semibold mb-1">Bienvenido</h1>
          <p className="text-fg/50 text-sm mb-8">Inicia sesión para reservar libros.</p>
          ...
          <p className="mt-6 text-sm text-fg/50 text-center">
            ¿No tienes cuenta?{' '}
            <Link to="/register" className="text-fg font-medium hover:underline underline-offset-2">
              Regístrate
            </Link>
          </p>
        </div>
